...
const containerVariant = cva(
    "",
    {
        variants: {
            variant: {
                default: "container px-2 w-full mx-auto sm:max-w-2xl sm:px-0 md:max-w-3xl lg:max-w-5xl xl:max-w-7xl",
...
const Container = ({ children, className, variant, ...props }: ContainerProps) => {
    return (
        <section {...props} className={cn(containerVariant({ variant }), className)}>
            {children}
        </section>
    )
}
